...
// 2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)
    ...
    const authorOfFirstComment = () => users.find(user => user.id === comments[0].userId);

    console.log(authorOfFirstComment());

// 3) Which user commented 'OK great thanks'?

    const userComment = () => users.find(user => user.id === comments.find(comment => comment.text === 'OK great thanks').userId);
    
    console.log(userComment());

// 4) Add the user's first and last name to each comment in the comments array

    const addUser = () => comments.map(comment => {
...
// 5) Get a list of the users who haven't commented
    ...
    const userWithNoComment = () => users.filter(user => !comments.find(comment => comment.userId === user.id));

    console.log(userWithNoComment());