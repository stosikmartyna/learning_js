...
//  Check whether every email address in the list of users is valid (get a true or false value)
    ...
    const emailValidation = () => usersData.every(user => /[^@]+@[^\.]+\..+/.test(user.email));
...
// https://en.wikipedia.org/wiki/Classful_network
  ...
  const usersWithIP = () => usersData.filter(({ip_address}) => ip_address.split('.')[0] < 128).length;
  
  console.log(usersWithIP());

// Exercise 03
// Find the user object that has the 'soundcloud' email address

    const userEmail = () => usersData.find(user => user.email.includes('soundcloud'));
...
// Replace all the items in the array with a string value of 'deleted'
  ...
  const replaceItems = () => usersData.fill('deleted');
